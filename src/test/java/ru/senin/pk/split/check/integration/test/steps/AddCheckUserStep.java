...
public class AddCheckUserStep extends ITAbstractStep<AddCheckUserStep> {
    ...
    private static final String PATH = "/checks/add_user";

    @Override
    protected ITParams executionImpl(ITParams inParams) {
        String origin = inParams.get(ITParam.ORIGIN);
        String url = origin + PATH + "?check_id=" + inParams.get(ITParam.CHECK_ID) + "&new_user_id=" + inParams.get(ITParam.USER_ID);
